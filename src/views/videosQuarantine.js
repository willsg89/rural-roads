...
const videos = [
  ...
  { title: "video-fortunate", src: "https://www.youtube.com/embed/QaN-e8D0t8c" },
  { title: "video-shortcut", src: "https://www.youtube.com/embed/9qB2ZrSvJDk" },
];

function VideosQuarantine() {
  const { t } = useTranslation();
  logEvent(analytics, "videos_quarantine_load");
  return (
    <div>
      ...
      <Row>
        {videos.map((video) => (
          <Col key={video.title} xs={12} md={6} style={{ padding: 10 }}>
            <Video
              onClick={() => logEvent(analytics, "videos_" + video.title + "_click")}
              title={video.title}
              mb
              src={video.src}
            />
